...
function getDependencies (manifest, opts, cb) {
  ...
  var pkgs = {}
  var deps = normaliseDeps(manifest[depType()] || {})
  ...
  var tasks = depNames.map(function (depName) {
    return function (cb) {
      ...
      getVersionsInfo(depName, opts, function (err, versionsInfo) {
        if (err) {
          // Registry client used to respond with an Error but currently (v6.5.1)
          // responds with a string. Hopefully in the future it'll switch back
          // https://github.com/npm/npm-registry-client/issues/112
          if (!opts.error.E404 && (err === '404 Not Found' || err.code === 'E404')) {
