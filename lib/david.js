...
// Debug counters for performance, issue #110
var failTimeObjectMissing
var countDonePkg

// An array containing names of problematic packages
var wrongInheritance

/*
  Helper functions for debugging issue #110
*/
function dbgM(stra) {
  return
  // console.log(stra)
...
// A higher level debug message. Used to inspect object inheritance of the
// 'data' object, assumed to have .sort function in getVersionsInfo()
function dbgH(stra) {
  console.log(stra)
}
...
function getVersionsInfo (name, opts, cb) {
  npm.load(opts.npm || {}, function (err) {
    ...
    npm.commands.view([name, 'versions', 'time'], true, function (err, data) {
      ...
      // Fix: remove 2 lines. Redundant, development-time helpers for fixing issue #110
      dbgM('Looking for all versions of package ' + name)
      dbgM(' => Now got currentVersion:         ' + currentVersion) 

      // `npm view 0 versions` returns {}
      if (!currentVersion) {
        dbgM(' => fail. Could not get versions for the package through npm')
        return cb(new Error('Failed to get versions for ' + name))
      }

      dbgM(' => npm view versions succeeded!')

      // Some packages simply don't have a time object
      if (data[currentVersion].time) {
        // Checking for proper inheritance: the .sort should be coming from Object prototype
        var typeCheckStr = null
        if (data[currentVersion].versions.sort) {
          typeCheckStr = typeof data[currentVersion].versions.sort
        }
        // Playing safe - the typeCheckStr might be a null        
        if (!typeCheckStr || typeCheckStr !== 'function') {
          wrongInheritance.push(name)
          dbgH('Failed inheritance in data object for package: ' + name)
        } else {
          dbgH('OK inheritance in data object for package:     ' + name)
        }
        ...
        versions = data[currentVersion].versions.sort(function (a, b) {
          ...
          countDonePkg++
          dbgM (' => Done: ' + countDonePkg + ' / timeMisses: ' + failTimeObjectMissing)
          return (a < b ? -1 : (a > b ? 1 : 0))
        })
      } else {
        failTimeObjectMissing++
        dbgH (' A missing time object for package: ' + name)
        dbgM (' => Done: ' + countDonePkg + ' / timeMisses: ' + failTimeObjectMissing)
...
module.exports.getUpdatedDependencies = function (manifest, opts, cb) {
  ...
  // Initialize the counters and array
  failTimeObjectMissing=0
  countDonePkg=0
  wrongInheritance=[]
