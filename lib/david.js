 ...
 * @param {Boolean} [options.dev] Consider devDependencies
 * @param {Boolean} [options.loose] Use loose option when querying semver
 * @param {Function<Error, Object>} callback Function that receives the results
 */
			...
			getDependency(depName, function(err, dep) {
				...
				if(err) {
					...
					if(pkgDepVersion != 'latest' && pkgDepVersion != '*') {
						...
						var range = semver.validRange(pkgDepVersion, options.loose) || '';
						...
						if(version) {
							...
							} else if(!semver.satisfies(version, range, options.loose)) {
								
								if(options.stable && semverext.gtr(version, range, options.loose)) {
