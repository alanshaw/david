  ...
  var tasks = depNames.map(function (depName) {
    return function (cb) {
      if (Object.prototype.toString.call(deps[depName]) !== "[object String]") {
        var er = new Error("Non-string dependency: " + deps[depName])
        er.code = "EDEPTYPE"

        if (!opts.error.EDEPTYPE) {
          pkgs[depName] = {required: deps[depName], warn: er}
        } else {
          error = er
        }
      ...
      if (isScm(deps[depName])) {
        var er = new Error("SCM dependency: " + deps[depName])
        er.code = "ESCM"
      ...
      getVersionsInfo(depName, opts, function (er, versionsInfo) {
        if (er) {
          if (!opts.error.E404 && er.code === "E404") {
            pkgs[depName] = {required: deps[depName], warn: er}
          } else {
            error = er
          }
